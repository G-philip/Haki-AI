...
export default function Navbar({ mode, userName = 'Guest', onSettings, onEditProfile, onChangePassword }) {
  ...
  return (
    <>
      ...
      {/* No top/bottom border, per explicit instruction. */}
      <div className="w-full bg-white flex items-center justify-between px-6 py-3 flex-shrink-0">
        ...
        <div className="flex items-center gap-2 text-[13px] text-gray-600">
          ...
          <span
            className={`status-dot inline-block w-2 h-2 rounded-full ${connected ? 'bg-green-500' : 'bg-gray-400'
              }`}
          ...
          {menuOpen && (
            <div className="absolute top-full right-0 mt-2 w-52 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden z-50">
              <button
                ...
                className="w-full text-left px-4 py-2.5 text-[13px] text-gray-700 hover:bg-gray-50 transition flex items-center gap-2"
              >
                ⚙️ Settings
              </button>
              <button
