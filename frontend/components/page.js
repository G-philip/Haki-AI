      ...
      {!userType ? (
        ...
        <>
          ...
          <div className="flex-1 relative min-w-0 min-h-0">
            {/* Scroll container now spans the FULL column, edge to edge
                (absolute inset-0), instead of being flex-1 alongside a
                flex-sibling input that would reserve its own row and
                shrink this to fit around it. Navbar stays sticky inside
                it exactly as before. */}
            <div className="chat-scroll absolute inset-0 overflow-y-auto">
            ...
            {/* Fade so message content doesn't hard-clip as it scrolls
                behind the floating input below. bottom-[78px] is a
                static approximation of the input's typical (single-line)
                height + its own bottom margin — it won't track perfectly
                if the textarea grows to multiple lines, but the input's
                own background covers the input area regardless, so a
                slightly-misaligned fade is a minor cosmetic gap, not a
                functional one. Revisit this value if the multi-line
                input case looks off. */}
            <div className="absolute left-0 right-0 bottom-[78px] h-8 bg-gradient-to-b from-white/0 to-white pointer-events-none z-10" />

            {/* Floating input: no longer a flex sibling reserving its own
                space — absolutely positioned over the scroll area, on
                top of the fade. See ChatArea's increased bottom padding,
                which is what keeps the LAST message clear of this instead
                of being hidden underneath it. */}
            <div ref={chatInputWrapperRef} className="absolute left-0 right-0 bottom-4 z-20">
              <ChatInput
                onSend={sendMessage}
                disabled={loading}
                needsMode={needsMode}
