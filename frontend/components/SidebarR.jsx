...
export default function Sidebar({
  ...
  // instead,the sidebar stays present as a narrow strip rather than
      ...
      <div className="w-14 h-full bg-[#0d9488] flex flex-col items-center py-4 gap-5 flex-shrink-0">
        ...
        {navButtons.map((btn) => (
          <button
            ...
            onClick={btn.onClick}
            title={btn.label}
            className="text-white font-bold hover:text-white transition"
          >
            {btn.icon}
          </button>
        ))}
  ...
  return (
    <div className="w-[270px] bg-[#0d9488] flex flex-col h-full flex-shrink-0">
      {/* Header */}
      ...
      <div className="flex flex-col gap-1 px-4">
        {navButtons.map((btn) => (
          <button
            key={btn.key}
            onClick={btn.onClick}
            className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-[14px] font-medium text-white hover:bg-white/10 transition text-left"
          >
      ...
      {/* Recents */}
      <div className="flex-1 overflow-y-hidden px-3 pt-0 pb-0 sidebar-history">
        <p className="text-[14px] text-white font-medium pt-1 pb-1 px-2">Recents</p>
        {Object.keys(grouped).length === 0 ? (
          <p className="text-[14px] text-white py-2 px-2">No conversations yet</p>
        ) : (
          Object.entries(grouped).map(([label, items]) => (
            <div key={label}>
              <p className="text-[12px] text-white font-medium pt-3 pb-1">{label}</p>
              {items.map((item) => (
